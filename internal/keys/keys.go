...
func create(db orm.DB) zip.TypedHandler[schema.Key, schema.Key] {
	return func(ctx context.Context, in *schema.Key) (*schema.Key, error) {
		...
		if k.Scope == schema.KeyScopePublish {
			...
			// ingest endpoint (compat resolve-key → /v1/iam/resolve-key), never as a principal.
			k.AccessSecret = ""
...
// its pk- half org-resolvable at the ingest endpoint — a privilege change disguised as an
