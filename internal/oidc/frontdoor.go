...
func authMethods(db orm.DB) zip.Handler {
	return func(c *zip.Ctx) error {
		...
		return httpx.Ok(c, map[string]any{
			...
			// sendVerificationCode itself refuses on; reading it here is what stops
			// the page rendering "email me a code" for a code that never arrives.
			"code":     app.EnableCodeSignin && codeDelivery() == nil,
