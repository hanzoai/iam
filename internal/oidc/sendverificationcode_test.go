...
// deliveryReason is the message a caller sees when delivery is impossible —
// read from the ONE authority so these tests never restate its wording.
func deliveryReason(t *testing.T) string {
	t.Helper()
	_, err := codeDelivery()
	if err == nil {
		t.Fatal("this test requires delivery to be impossible")
	}
	return err.Error()
}
...
func TestSendVerificationCode_RefusedAndStoresNothingWithoutDelivery(t *testing.T) {
	...
	if msg, _ := env["msg"].(string); msg != deliveryReason(t) {
		t.Errorf("msg = %q, want the codeDelivery reason %q", msg, deliveryReason(t))
	}
