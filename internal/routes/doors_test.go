...
func TestControlDoors_RequireABearer(t *testing.T) {
	...
	// case-insensitively and strips EVERY trailing slash, so a gate that compares the
	// raw path — or one that strips a single slash — decides about a different string
	// than the router routed on. Each address is therefore driven in the spelling it
	// is written in AND in the ones that reach the same handler: an uppercase one, a
	// slash-suffixed one, and a doubled- and tripled-slash one, since those were the
	// shapes that walked past.
	doors := []struct {
		method, path string
	}{
		{"POST", "/mcp"},
