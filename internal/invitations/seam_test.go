...
// The invitations CRUD surface driven at the handler seam: Get, Create, Update and
...
// mustCreate seeds one invitation through the real Create path and returns it.
func mustCreate(t *testing.T, h *Handler, owner, name string) *schema.Invitation {
	t.Helper()
	out, err := h.Create(context.Background(), &Input{Owner: owner, Name: name, DisplayName: "original"})
	if err != nil {
...
func TestCreateRejectsDuplicate(t *testing.T) {
	h, _ := newHandler(t)
	mustCreate(t, h, "hanzo", "welcome")
	_, err := h.Create(context.Background(), &Input{Owner: "hanzo", Name: "welcome", DisplayName: "again"})
...
// key, an unstamped Create is filed with a server createdTime, and an Update
// rewrites the mutable fields while identity and the created stamp hold.
func TestGetReadsBackWhatCreateWrote(t *testing.T) {
	h, _ := newHandler(t)
	ctx := context.Background()
	created := mustCreate(t, h, "hanzo", "welcome")
	if created.CreatedTime == "" {
		t.Fatal("Create left createdTime empty — an unstamped add must be stamped by the server")
	}

	got, err := h.Get(ctx, &Ref{Owner: "hanzo", Name: "welcome"})
	if err != nil {
		t.Fatalf("get: %v", err)
	}
	...
	up, err := h.Update(ctx, &Input{Owner: "hanzo", Name: "welcome", DisplayName: "revised", Quota: 5})
	if err != nil {
		t.Fatalf("update: %v", err)
	}
	if up.DisplayName != "revised" || up.Quota != 5 {
...
// TestCreateHonoursSuppliedStamp: a Create that names its own createdTime keeps it
// rather than overwriting with server time — the stamp is defaulted only when blank.
func TestCreateHonoursSuppliedStamp(t *testing.T) {
	h, _ := newHandler(t)
	const stamp = "2020-01-02T03:04:05Z"
	out, err := h.Create(context.Background(), &Input{Owner: "hanzo", Name: "dated", CreatedTime: stamp})
	if err != nil {
		t.Fatalf("create: %v", err)
	}
	if out.CreatedTime != stamp {
		t.Fatalf("createdTime = %q, want the supplied %q", out.CreatedTime, stamp)
	}
}
...
func TestStoreFaultIsInternal(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"Get", func() error { _, err := h.Get(ctx, &Ref{Owner: "hanzo", Name: "welcome"}); return err }},
		{"Create", func() error {
			_, err := h.Create(ctx, &Input{Owner: "hanzo", Name: "fresh"})
			return err
		}},
		{"Update", func() error {
			_, err := h.Update(ctx, &Input{Owner: "hanzo", Name: "welcome"})
			return err
		}},
		{"Delete", func() error { _, err := h.Delete(ctx, &Ref{Owner: "hanzo", Name: "welcome"}); return err }},
	} {
...
func TestWriteFaultIsInternal(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"Create", func() error {
			_, err := hf.Create(ctx, &Input{Owner: "hanzo", Name: "fresh"})
			return err
		}},
		{"Update", func() error {
			_, err := hf.Update(ctx, &Input{Owner: "hanzo", Name: "welcome"})
			return err
		}},
		{"Delete", func() error { _, err := hf.Delete(ctx, &Ref{Owner: "hanzo", Name: "welcome"}); return err }},
	} {
		t.Run(tc.name, func(t *testing.T) { wantStatus(t, tc.call(), 500) })
	}
}
