...
// address-authority round trip lives in workspaces_test.go and drives the router;
// here the concern is the branch logic each handler owns — the required-key
// refusal, the not-found and conflict mappings, the store-error fallthrough, and
// the Organization default — none of which a routed request can reach (an empty
// path segment never binds :owner or :name, and List's own-org query needs a
// principal the Guard sets).
package workspaces
...
// openStore opens a fresh owner-scoped sqlite store, torn down with the test.
	...
	for op, run := range ops {
		for _, k := range keys {
			t.Run(op+"/"+k.name, func(t *testing.T) {
				wantStatus(t, run(k.owner, k.wksp), 400)
			})
		}
	}
}

// Create stamps CreatedTime when the body omits it, keeps it when supplied,
// carries every mutable field through apply onto the stored workspace, and
// defaults Organization to the owner when the body names none.
func TestCreateStampsAndAppliesFields(t *testing.T) {
	h := &Handler{db: openStore(t)}
	ctx := context.Background()

	// Organization omitted: it defaults to the owner, and an omitted CreatedTime
	// is stamped.
	got, err := h.Create(ctx, &Input{Owner: "hanzo", Name: "default-org"})
	if err != nil {
	...
	case w.DisplayName != in.DisplayName,
		...
		!w.IsDefault:
		t.Fatalf("apply did not carry the input onto the workspace: %+v", w)
	}
}
...
func TestCreateRejectsDuplicate(t *testing.T) {
	h := &Handler{db: openStore(t)}
	ctx := context.Background()
	if _, err := h.Create(ctx, &Input{Owner: "hanzo", Name: "w"}); err != nil {
		t.Fatalf("first create: %v", err)
	}
	wantStatus(t, func() error {
		_, err := h.Create(ctx, &Input{Owner: "hanzo", Name: "w"})
		return err
	}(), 409)
}

// Reading, updating, or deleting a workspace no org has is a 404, not a 500 —
// mapErr distinguishes the missing row from a store failure.
func TestMissingWorkspaceIsNotFound(t *testing.T) {
	h := &Handler{db: openStore(t)}
	ctx := context.Background()
...
func TestCreateReadUpdateDelete(t *testing.T) {
	...
	if _, err := h.Create(ctx, &Input{Owner: "hanzo", Name: "w", DisplayName: "one"}); err != nil {
		t.Fatalf("create: %v", err)
	}
	got, err := h.Get(ctx, &Ref{Owner: "hanzo", Name: "w"})
	if err != nil || got.DisplayName != "one" {
		t.Fatalf("get: %v %+v", err, got)
	}
	up, err := h.Update(ctx, &Input{Owner: "hanzo", Name: "w", DisplayName: "two"})
	if err != nil || up.DisplayName != "two" {
		t.Fatalf("update: %v %+v", err, up)
	}
	del, err := h.Delete(ctx, &Ref{Owner: "hanzo", Name: "w"})
	if err != nil || !del.Deleted {
		t.Fatalf("delete: %v %+v", err, del)
	}
	if _, err := h.Get(ctx, &Ref{Owner: "hanzo", Name: "w"}); !errors.As(err, new(*zip.HTTPError)) {
