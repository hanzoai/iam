...
func provisionCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			// AUTHORITY it is given, and whether a credential was actually
			// delivered — "converged", "privileged" and "usable" are three different
			// facts and a line that shows only the first hides the other two. The
			// admin bit is printed for every account, including the false ones: a
			// grant is only reviewable beside the accounts that do not have it. It
