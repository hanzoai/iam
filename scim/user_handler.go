...
func UpdateScimUserByPatchOperation(id string, ops []scim.PatchOperation) (r scim.Resource, err error) {
	...
	for _, op := range ops {
		...
		// PatchOperationAdd and PatchOperationReplace is same in IAM, just replace the value
		...
		case "photos":
			defaultV := AnyArray{AnyMap{"value": ""}}
			vs := ToAnyArray(value, defaultV) // e.g. [{"value": "https://hanzo.ai/img/hanzo-logo.svg"}]
			if len(vs) > 0 {
				v := ToAnyMap(vs[0])
