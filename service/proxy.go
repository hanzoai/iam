...
// Portions Copyright 2023 The Casdoor Authors. All Rights Reserved.
//
...
func handleRequest(w http.ResponseWriter, r *http.Request) {
	...
	if site == nil {
		...
		if strings.HasSuffix(r.Host, ".hanzo.ai") && r.RequestURI == "/health-ping" {
	...
	// oAuth proxy
	if site.IamApplication != "" {
		// handle oAuth proxy
		cookie, err := r.Cookie("iam_access_token")
		...
		if cookie == nil {
			// not logged in
			redirectToIAM(iamClient, w, r)
			return
		} else {
			_, err = iamClient.ParseJwtToken(cookie.Value)
			if err != nil {
				responseError(w, "IAM WAF error: iamClient.ParseJwtToken() error: %s", err.Error())
				return
			}
		}
	}
	...
	host := site.GetHost()
	if host == "" {
		responseError(w, "IAM WAF error: targetUrl should not be empty for host: %s, site = %v", r.Host, site)
		return
	}
