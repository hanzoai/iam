...
to resolve `iam.{env}.satschel.com` automatically and fetches the
...
# In Liquidity dev
export IAM_ADDR=https://iam.dev.satschel.com
...
- **Liquidity wrapper**: [`liquidityio/cli`](https://github.com/liquidityio/cli) — `liquid iam redirect add <client-id> <url> ...` adds `--env=dev|test|main` resolution on top of the upstream CLI.
