...
to resolve `iam.{env}.` automatically and fetches the
...
export IAM_TOKEN=<bearer from any  pod env>

# In Liquidity dev
export IAM_ADDR=https://iam.dev.
...
$ iam app get vcc-exchange-client-id
{
  ...
  "redirectUris": [
    "https://vcc.dev./callback",
    "https://vcc.dev./auth/callback"
  ],
  ...
}
```
...
- **Liquidity wrapper**: [`/cli`](https://github.com//cli) — `liquid iam redirect add <client-id> <url> ...` adds `--env=dev|test|main` resolution on top of the upstream CLI.
...
- ❌ Hand-rolled secret tokens. The bearer always comes from a deployed `` pod or the IAM admin login. Don't paste tokens into chat.
