...
`partner/iam`, `onyx-plus/iam`, `securegate/iam`, …) must follow these
...
| **Consumer app slug** | **`<consumer-org>-<appname>`** — e.g. `hanzo-brain`, `liquidity-id`, `onyxplus-verify`, `zoo-ngo`, `lux-cloud` |
| Consumer org | `<consumer-org>` — e.g. `hanzo`, `liquidity`, `onyxplus`, `zoo`, `lux` |
...
TS hostname switches. No `if (host === 'iam.dev.example.internal')` anywhere.
