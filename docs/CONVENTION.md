...
`tenantio/iam`, `onyx-plus/iam`, `securegate/iam`, …) must follow these
...
| **Consumer app slug** | **`<consumer-org>-<appname>`** — e.g. `hanzo-brain`, `tenant-app`, `onyxplus-verify`, `zoo-ngo`, `lux-cloud` |
| Consumer org | `<consumer-org>` — e.g. `hanzo`, `tenant`, `onyxplus`, `zoo`, `lux` |
