...
func (c *Client) GetOAuthToken(code string, state string, opts ...OAuthOption) (*oauth2.Token, error) {
	...
	config := oauth2.Config{
		...
		Endpoint: oauth2.Endpoint{
			AuthURL:   fmt.Sprintf("%s/v1/iam/oauth/authorize", c.Endpoint),
			TokenURL:  fmt.Sprintf("%s/v1/iam/oauth/access_token", c.Endpoint),
...
func (c *Client) RefreshOAuthToken(refreshToken string, opts ...OAuthOption) (*oauth2.Token, error) {
	...
	config := oauth2.Config{
		...
		Endpoint: oauth2.Endpoint{
			AuthURL:   fmt.Sprintf("%s/v1/iam/oauth/authorize", c.Endpoint),
			TokenURL:  fmt.Sprintf("%s/v1/iam/oauth/refresh_token", c.Endpoint),
			AuthStyle: oauth2.AuthStyleInParams,
		},
		// RedirectURL: redirectUri,
		Scopes: nil,
