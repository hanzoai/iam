...
func getIAMCertMap() (map[string]*Cert, error) {
	certs, err := GetCerts("")
	if err != nil {
...
func getIAMApplicationMap() (map[string]*Application, error) {
	iamCertMap, err := getIAMCertMap()
	if err != nil {
		return nil, err
	}
	...
	for _, application := range applications {
		if application.Cert != "" {
			if cert, ok := iamCertMap[application.Cert]; ok {
...
func refreshSiteMap() error {
	applicationMap, err := getIAMApplicationMap()
	if err != nil {
		fmt.Println(err)
	}
	...
	for _, site := range sites {
		if applicationMap != nil {
			if site.IamApplication != "" && site.ApplicationObj == nil {
				if v, ok2 := applicationMap[site.IamApplication]; ok2 {
