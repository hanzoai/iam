...
// DEPRECATED: ProductInfo is retained for backward compatibility with the Cart column.
// Billing is now managed by Commerce (billing.hanzo.ai).
...
func AddUser(user *User, lang string) (bool, error) {
	...
	// applyDefaultApprovalStatus leaves trusted/provisioned identities approved and
	// keeps fail-open intact for EXISTING users (untouched rows have no property).
