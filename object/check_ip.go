...
// Portions Copyright 2024 The Casdoor Authors. All Rights Reserved.
//
...
func CheckEntryIp(clientIp string, user *User, application *Application, organization *Organization, lang string) error {
	...
	if user != nil {
		...
		if err != nil {
			return fmt.Errorf("%s", err.Error()+user.Name)
		}
	}
	...
	if application != nil {
		...
		if err != nil {
			...
			return fmt.Errorf("%s", err.Error()+application.Name)
		} else {
	...
	if organization != nil {
		...
		if err != nil {
			...
			return fmt.Errorf("%s", err.Error()+organization.Name)
		} else {
