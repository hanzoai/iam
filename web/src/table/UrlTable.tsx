...
function UrlTable({title, table, onUpdateTable}) {
  const updateTable = useCallback((tbl) => {
    onUpdateTable(tbl);
  }, [onUpdateTable]);

  const updateField = useCallback((tbl, index, value) => {
    tbl[index] = value;
    updateTable([...tbl]);
  }, [updateTable]);
  ...
  const addRow = useCallback((tbl) => {
    let newTable = tbl ?? [];
    newTable = Setting.addRow(newTable, "");
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/10 bg-white/[0.02]">
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">{i18next.t("application:Redirect URL")}</th>
              ...
              <tr key={i} className="border-b border-white/[0.05] hover:bg-white/[0.02]">
                <td className="px-4 py-2">
                  <div className="flex items-center gap-2">
                    ...
                    <input className="w-full bg-white/5 border border-white/10 rounded px-2 py-1 text-sm text-white" value={row} onChange={e => updateField(table, i, e.target.value)} />
                  </div>
                </td>
