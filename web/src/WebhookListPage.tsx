...
function WebhookListPage(props) {
  ...
  const fetchData = useCallback((params = {}) => {
    ...
    if (params.contentType !== undefined && params.contentType !== null) { field = "contentType"; value = params.contentType; }
    setLoading(true);
    WebhookBackend.getWebhooks("admin", Setting.isDefaultOrganizationSelected(account) ? "" : Setting.getRequestOrganization(account), pag.current, pag.pageSize, field, value, sf, so)
      .then((res) => {
        setLoading(false);
  ...
  function addWebhook() {
    ...
    const w = {
      owner: "admin", name: `webhook_${randomName}`, createdTime: moment().format(), organization: organizationName,
      url: "https://example.com/callback", method: "POST", contentType: "application/json",
      headers: [], events: ["signup", "login", "logout", "update-user"], isEnabled: true,
    };
    WebhookBackend.addWebhook(w).then((res) => {
      if (res.status === "ok") { history.push({pathname: `/webhooks/${w.name}`, mode: "add"}); Setting.showMessage("success", i18next.t("general:Successfully added")); }
  ...
  function deleteWebhook(i) {
    WebhookBackend.deleteWebhook(data[i]).then((res) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-white">{i18next.t("general:Webhooks")}</h2>
        <div className="flex items-center gap-2">
          <span className="text-sm text-zinc-400">{i18next.t("general:{total} in total").replace("{total}", String(pagination.total))}</span>
          <Button size="sm" onClick={addWebhook}><Plus className="w-4 h-4 mr-1" />{i18next.t("general:Add")}</Button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-zinc-800 bg-zinc-900/50">
              ...
              <th className="text-left px-4 py-3 text-zinc-400 font-medium">{i18next.t("webhook:Content type")}</th>
              <th className="text-left px-4 py-3 text-zinc-400 font-medium">{i18next.t("webhook:Events")}</th>
              ...
              {loading ? (<tr><td colSpan={9} className="px-4 py-8 text-center text-zinc-500">Loading...</td></tr>) :
               ...
               data.map((record, index) => (
                <tr key={`${record.owner}/${record.name}`} className="border-b border-zinc-800/50 hover:bg-zinc-900/30">
                  <td className="px-4 py-3"><Link to={`/webhooks/${record.name}`} className="text-blue-400 hover:text-blue-300">{record.name}</Link></td>
                  ...
                  <td className="px-4 py-3"><a target="_blank" rel="noreferrer" href={record.url} className="text-blue-400 hover:text-blue-300">{Setting.getShortText(record.url)}</a></td>
                  <td className="px-4 py-3 text-zinc-300">{record.method}</td>
                  <td className="px-4 py-3 text-zinc-300">{record.contentType}</td>
                  <td className="px-4 py-3 text-zinc-300">{record.events?.join(", ")}</td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => history.push(`/webhooks/${record.name}`)}><Pencil className="w-3 h-3 mr-1" />{i18next.t("general:Edit")}</Button>
                      <Button variant="destructive" size="sm" onClick={() => { if (window.confirm(`${i18next.t("general:Sure to delete")}: ${record.name} ?`)) deleteWebhook(index); }}>
