...
function InvitationListPage(props) {
  ...
  const fetchData = useCallback((params = {}) => {
    ...
    setLoading(true);
    InvitationBackend.getInvitations(Setting.isDefaultOrganizationSelected(account) ? "" : Setting.getRequestOrganization(account), pag.current, pag.pageSize, field, value, sf, so)
      .then((res) => {
        setLoading(false);
  ...
  function addInvitation() {
    ...
    const inv = {owner, name: `invitation_${randomName}`, createdTime: moment().format(), updatedTime: moment().format(), displayName: `New Invitation - ${randomName}`, code, defaultCode: code, quota: 1, usedCount: 0, application: "All", username: "", email: "", phone: "", signupGroup: "", state: "Active"};
    InvitationBackend.addInvitation(inv).then((res) => {
      if (res.status === "ok") { history.push({pathname: `/invitations/${inv.owner}/${inv.name}`, mode: "add"}); Setting.showMessage("success", i18next.t("general:Successfully added")); }
  ...
  function deleteInvitation(i) {
    InvitationBackend.deleteInvitation(data[i]).then((res) => {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-semibold text-white">{i18next.t("general:Invitations")}</h2>
        <div className="flex items-center gap-2">
          <span className="text-sm text-zinc-400">{i18next.t("general:{total} in total").replace("{total}", String(pagination.total))}</span>
          <Button size="sm" onClick={addInvitation}><Plus className="w-4 h-4 mr-1" />{i18next.t("general:Add")}</Button>
        </div>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-zinc-800 bg-zinc-900/50">
              ...
              <th className="text-left px-4 py-3 text-zinc-400 font-medium">{i18next.t("invitation:Code")}</th>
              <th className="text-left px-4 py-3 text-zinc-400 font-medium">{i18next.t("invitation:Quota")}</th>
              <th className="text-left px-4 py-3 text-zinc-400 font-medium">{i18next.t("invitation:Used count")}</th>
              ...
              {loading ? (<tr><td colSpan={9} className="px-4 py-8 text-center text-zinc-500">Loading...</td></tr>) :
               ...
               data.map((record, index) => (
                <tr key={`${record.owner}/${record.name}`} className="border-b border-zinc-800/50 hover:bg-zinc-900/30">
                  <td className="px-4 py-3"><Link to={`/invitations/${record.owner}/${record.name}`} className="text-blue-400 hover:text-blue-300">{record.name}</Link></td>
                  ...
                  <td className="px-4 py-3 text-zinc-300">{record.quota}</td>
                  <td className="px-4 py-3 text-zinc-300">{record.usedCount}</td>
                  <td className="px-4 py-3"><Link to={`/applications/${record.owner}/${record.application}`} className="text-blue-400 hover:text-blue-300">{record.application}</Link></td>
                  <td className="px-4 py-3"><span className={`px-2 py-0.5 rounded text-xs ${record.state === "Active" ? "bg-green-900/50 text-green-400" : "bg-zinc-800 text-zinc-500"}`}>{record.state}</span></td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      <Button variant="outline" size="sm" onClick={() => history.push(`/invitations/${record.owner}/${record.name}`)}><Pencil className="w-3 h-3 mr-1" />{i18next.t("general:Edit")}</Button>
                      <Button variant="destructive" size="sm" onClick={() => { if (window.confirm(`${i18next.t("general:Sure to delete")}: ${record.name} ?`)) deleteInvitation(index); }}>
