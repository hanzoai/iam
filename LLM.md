...
1. Browser hits `https://iam.{env}.example.internal/v1/iam/login`
...
```bash
curl -X POST https://iam.dev.example.internal/v1/iam/login/oauth/access_token \
...
|---|---|---|---|
| devnet | `https://iam.dev.example.internal` | `partner/iam:dev` | Base/SQLite (PVC) |
| testnet | `https://iam.test.example.internal` | `partner/iam:test` | Base/SQLite (PVC) |
| mainnet | `https://iam.main.example.internal` | `partner/iam:main` | Base/SQLite (PVC) |
| local | `http://localhost:8000` | `partner/iam:dev` | SQLite (volume) |
...
- `us-docker.pkg.dev/liquidity-registry/partner/iam:{tag}` (for Liquidity)
